  ...
  return (
    <div class="container mx-auto flex flex-col items-center justify-center h-full text-center">
      ...
      <div class="mt-10 flex gap-5">
        <For
          each={LinkData.filter((f) => ["github", "linkedin"].includes(f.name))}
        >
          {(item) => {
            ...
            return (
              <a
                ...
                onClick={() => trackEvent("social_click", name)}
              >
                <button class="relative overflow-hidden text-[#e4ecff] text-4xl p-3 rounded-2xl bg-[rgba(255,255,255,0.1)] border-[rgba(255,255,255,0.2)] border shadow-2xl transition-all duration-300 hover:bg-[rgba(255,255,255,0.2)] hover:cursor-pointer hover:scale-105">
                  <span class="absolute inset-0 bg-gradient-to-r from-transparent via-[rgba(255,255,255,0.3)] to-transparent opacity-0 hover:opacity-100 transition-all duration-300"></span>
                  <Icon />
                </button>
              </a>
            );
        ...
        {(item) => {
          ...
          return (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              onClick={() => trackEvent("resume_download", "resume_button")}
            >
